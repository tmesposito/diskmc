...
#Number of zones : 1 zone = 1 density structure + corresponding grain properties
  ...
  {zone[scale_height]:<5.3f}  {zone[reference_radius]:<6.3f} {zone[debris_disk_vertical_profile_exponent]:<6.2f}          scale height, reference radius (AU), unused for envelope, vertical profile exponent (only for debris disk)
  {zone[r_in]:<6.3f}  {zone[edge]:<6.3f} {zone[r_out]:<6.3f} {zone[r_critical]:<6.3f}  Rin, edge, Rout, Rc (AU) Rc is only used for tappered-edge & debris disks (Rout set to 8*Rc if Rout==0)
